    ...
    fun level_1(option:String){
        firestore.collection("Exercise")
            .addSnapshotListener { querySnapshot, _ ->
                ...
                for (snapshot in querySnapshot!!.documents) {
                    if(snapshot.getString("level") == "초보자" && snapshot.getString("method") == option){
                        var item = snapshot.toObject(Exercise::class.java)
                        exercise.add(item!!)
                    }
                }
    ...
    fun level_2(option:String){
        firestore.collection("Exercise")
            .addSnapshotListener { querySnapshot, _ ->
                ...
                for (snapshot in querySnapshot!!.documents) {
                    if(snapshot.getString("level") == "중급자" && snapshot.getString("method") == option){
                        var item = snapshot.toObject(Exercise::class.java)
                        exercise.add(item!!)
                    }
                }
    ...
    fun level_3(option:String){
        firestore.collection("Exercise")
            .addSnapshotListener { querySnapshot, _ ->
                ...
                for (snapshot in querySnapshot!!.documents) {
                    if(snapshot.getString("level") == "숙련자" && snapshot.getString("method") == option){
                        var item = snapshot.toObject(Exercise::class.java)
                        exercise.add(item!!)
                    }
                }
